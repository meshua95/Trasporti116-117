...
/*
        PatientPersonalData pd = new PatientPersonalData("Meshua", "Galassi", LocalDate.of(1995, 12,05), new PatientResidence(new Address("via selice"), new HouseNumber(2), new City("Massa Lombarda"), new District("RA"), new PostalCode(48024)));

        PatientDigitalTwin.createPatient(new PatientFiscalCode("GLSMSH95T45E730V"), pd, new HealthState("buono"), Autonomy.AUTONOMOUS);
        */
        BookingRoute bookingRoute = new BookingRoute(new BookingLocation(new Address("IV Settembre"),new HouseNumber(13),new City("Cesena"), new District("FC"), new PostalCode(47521)),
                new BookingLocation(new Address("corso cavour"),new HouseNumber(189),new City("Cesena"), new District("FC"), new PostalCode(47521)));

        BookingTransportId bookingId = BookingDigitalTwin.createBookingTransport(LocalDateTime.now(), bookingRoute, new PatientFiscalCode("GLSMSH95T45E730V"), idRequest);
